...
class GithubCalls:
    ...
    def run_github_search(self, search_query, extension, org=[], repo=[], filter_archived=False, filter_forks=False):
        ...
        params: org - list
        params: repo - list
        params: filter_archived - bool - Filter out archived repositories
        params: filter_forks - bool - Filter out forked repositories
        ...
        if additional_qualifiers:
            try:
                response = requests.get(
                    self._base_url,
                    params={
                        "q": f"{search_query} {additional_qualifiers}{filter_qualifiers}",
                        "order": "desc",
                        "sort": "indexed",
                        "per_page": 100,
                    },
            ...
            try:
                response = requests.get(
                    self._base_url,
                    params={
                        "q": f"{search_query}{filter_qualifiers}",
                        "order": "desc",
                        "sort": "indexed",
                        "per_page": 100,
                    },
